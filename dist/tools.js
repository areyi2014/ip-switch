...
export function registerTools(server) {
    ...
    server.tool('rotate_ip_and_update_dns', 'One-click: rotate a saved profile instance IP and update its bound subdomain DNS. ' +
        ...
        try {
            ...
            const rotateResult = await adapter.rotateIp(profile.instanceId, profile.region, profile.credentials);
            ...
            // Step 4: Update DNS
            const dnsResult = await updateDns(cf, profile.subdomain, rotateResult.newIp, profile.proxied);
...
function formatError(err) {
    ...
    if (err instanceof CloudflareError) {
        return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: err.message, statusCode: err.statusCode }) }],
            isError: true,
        };
    }
